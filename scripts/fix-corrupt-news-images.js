// Fix 4 corrupt news images: re-download from WinWin originals, upload fresh, update DB, delete corrupt files
...
const FIXES = [
  { id: 'a561ee22-34d1-498e-9b4b-05b638b174e5', url: 'https://assets.winwin.com/2026-08/image-1785622749.jpg' },
  { id: 'f6625129-f8e0-4029-ba26-92db403bcb2b', url: 'https://assets.winwin.com/2026-08/image-1785619475.jpeg' },
  { id: '23989aeb-ceb7-403a-86f3-e7d60690abad', url: 'https://assets.winwin.com/2026-08/image-1785618119.jpg' },
  { id: '48042274-ef03-4349-b5e1-0db127be7626', url: 'https://assets.winwin.com/2026-08/image-1785617867.jpeg' },
];
...
(async () => {
  for (const fix of FIXES) {
    const { data: news, error: ne } = await sup.from('news').select('id, title, image_url').eq('id', fix.id).single();
    if (ne || !news) { console.log('❌ news not found', fix.id, ne?.message); continue; }
    console.log('\n🔧', news.title.substring(0, 60));
    console.log('   old image_url:', news.image_url);
    ...
    // 3. Update DB
    const { error: dbe } = await sup.from('news').update({ image_url: urlData.publicUrl }).eq('id', fix.id);
