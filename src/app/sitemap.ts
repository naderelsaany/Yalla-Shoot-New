...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const matchUrls: MetadataRoute.Sitemap = matches?.map((m: Partial<Match>) => {
    ...
    return {
      ...
      lastModified: new Date(m.updated_at || m.created_at || m.match_date || new Date()).toISOString(),
      changeFrequency: (isFinished ? 'yearly' : 'always') as "yearly" | "always",
      priority: isFinished ? 0.5 : 0.9,
    };
  }) || [];
